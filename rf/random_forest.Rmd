...
# EDIT 'mask' TO INCLUDE OR EXCLUDE PATCHES IN ELEVATION MASK
...
show_yearly_breakdown <- TRUE
if (show_yearly_breakdown) {
  time_series <- data.frame(matrix(0,length(year_list),length(unique(y[,1]))))
  rownames(time_series) <- year_list
  colnames(time_series) <- dist_labels
  for (i in 1:nrow(y)) {
    row <- match(years[i,1],year_list)
    col <- match(y[i,1],disturbances)
    time_series[row,col] <- time_series[row,col]+1
  }
  kable(time_series,align="l",caption="Disturbances over the years")
}
...
```{r more plotting,fig.align='center'}
