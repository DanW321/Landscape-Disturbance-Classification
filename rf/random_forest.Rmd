...
avalanche_train <- confusion_train["Avalanche","Error"]
avalanche_test <- confusion_test["Avalanche","Error"]
toppling_train <- confusion_train["Tree Toppling","Error"]
...
#### Training New Forest
```{r retraining}
# trains a forest with just avalanches and tree topples
dists_v2 <- c("Avalanche","Tree Toppling")
x_v2 <- x[y[,1]%in%dists_v2,]
y_v2 <- y[y[,1]%in%dists_v2,,drop=FALSE]
x_train_v2 <- data.frame(matrix(0,0,ncol(x_v2)))
y_train_v2 <- data.frame(matrix("",0,1))
x_test_v2 <- data.frame(matrix(0,0,ncol(x_v2)))
y_test_v2 <- data.frame(matrix("",0,1))
# EDIT 'split' TO CHANGE THE TRAINING SPLIT
split <- 0.75
for (disturbance in dists_v2) {
  x_subset <- x_v2[y_v2[,1]==disturbance,]
  y_subset <- y_v2[y_v2[,1]==disturbance,,drop=FALSE]
  num_rows <- nrow(x_subset)
  index_subset <- sample.int(num_rows,num_rows*split)
  x_train_v2 <- rbind(x_train_v2,x_subset[index_subset,])
  y_train_v2 <- rbind(y_train_v2,y_subset[index_subset,,drop=FALSE])
  x_test_v2 <- rbind(x_test_v2,x_subset[-index_subset,])
  y_test_v2 <- rbind(y_test_v2,y_subset[-index_subset,,drop=FALSE])
}
...
kable(confusion_test,align="l",caption=paste
      ("confusion matrix for test set with ",ncol(x_train_subset_v2),
        " variables (avg class error: ",class_error_test,")",sep=""))
avalanche_train_v2 <- confusion_train["Avalanche","Error"]
avalanche_test_v2 <- confusion_test["Avalanche","Error"]
toppling_train_v2 <- confusion_train["Tree Toppling","Error"]
